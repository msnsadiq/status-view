...
class StatusView extends StatefulWidget {
  StatusView({Key? key, required this.id}) : super(key: key);
  int id;
...
class _StatusViewState extends State<StatusView> {
  ...
  void initState() {
    ...
    BlocProvider.of<StatusBloc>(context)
        .add(FetchImage(personId: widget.id.toInt()));
    //  badgeHandling();
    ...
    return WillPopScope(
      onWillPop: () async {
        return await Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (ctx) => ScreenStatus()));
      },
      child: Scaffold(
          floatingActionButton: FloatingActionButton(
            ...
            onPressed: () async {
              ...
              prfs.clear();
              BlocProvider.of<StatusBloc>(context)
                  .add(FetchImage(personId: widget.id));
            },
            child: Icon(
              Icons.refresh,
          ...
          body: BlocBuilder<StatusBloc, StatusState>(builder: (context, state) {
            ...
            if (state is ImageLoaded) {
              filterList = BlocProvider.of<StatusBloc>(context).filterList;
              return Column(
                    ...
                    Container(
                        ...
                        child: PageView.builder(
                            onPageChanged: (index) async {
                              ...
                              customList.add(index.toString());

                              prfs.setStringList('seen', customList);
                              print('@#@#${prfs.getStringList('seen')}');
                            },
                            itemCount: filterList.length,
                            controller: pageController,
                            itemBuilder: (ctx, index) {
                              return filterList[index].image == null ||
                                      filterList[index].image!.isEmpty
                                  ? Center(child: Text("Image not found"))
                                  : Image.network(
                                      filterList[index].image.toString());
                            })),
                  ]);
            }
            if (state is ImageError) {
              return RefreshIndicator(
                onRefresh: () async {
                  return BlocProvider.of<StatusBloc>(context)
                      .add(FetchImage(personId: widget.id.toInt()));
                },
